class Php < Formula
  ...
  def install
    ...
    if build.with? "php-embed"
      args << "--enable-embed=static"
      args.delete '--enable-fpm'
      args.delete_if { |arg| arg =~ /^--with-fpm-(user|group)\b.+$/ }
      args.delete_if { |arg| arg =~ /^--with-apxs2\b.+$/ }
    end
    ...
    if build.with? "php-embed"
      config_files.delete "sapi/fpm/php-fpm.conf"
      config_files.delete "sapi/fpm/www.conf"
    end
    ...
    unless build.with? "php-embed" || (var/"log/php-fpm.log").exist?
      (var/"log").mkpath
  ...
  #TODO disable plist when built with php-embed
